    ...
    for e in elements:
        ...
        if e["type"] == "text":
            ...
            if source == "spline":
                ...
                raw = input("  New text (Enter to keep): ").strip()
            if raw:
                e["new_value"] = raw
                changed += 1
                if " / " in raw:
            ...
            raw = input("  Your file path (Enter for placeholder): ").strip().strip('"').strip("'")
            if raw:
            ...
            raw = input("  New hex color, e.g. #ff3300 (Enter to keep): ").strip()
            if raw:
